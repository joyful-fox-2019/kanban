<template>
  ...
  >
    <b-card-text v-for="data in dataDone" :key="data.id">
       <b-card
      :header="data.title"
      header-tag="header"
    >
     ...
     <b-modal ref="my-modal" hide-footer title="Detail :">
      <div class="d-block" v-for="(detail, i) in detailData" :key="i">
        ...
        <div style="display: flex; justify-content: space-between;">
          <b-button @click="goDoing(detail.id)" variant="dark">Doing</b-button>
...
export default {
  ...
  methods: {
    readData () {
      Swal.showLoading()
      db.collection('listtodo').where('status', '==', 'done')
        .onSnapshot((querySnapshot) => {
          Swal.close()
          this.dataDone = []
          querySnapshot.forEach((doc) => {
            this.dataDone.push({ id: doc.id, ...doc.data() })
          })
        })
    },
    showModal (id) {
  ...
  computed: {
    detailData () {
      return this.dataDone.filter(data => {
        return data.id
